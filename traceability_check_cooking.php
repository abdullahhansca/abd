...
if(isset($_REQUEST['submit']))
{
  ...
  insert_traceability_cooking($date1,$time1,$initial);
           ...
           <div class="col-md-12 col-lg-12 col-sm-12 ">
             <form action="" method="post">
              <table class="table table-bordered table-head-bg-info table-bordered-bd-info mt-4">
                <?php
                  if($ch_trc_flag==1 && $_SESSION['u_type']!="admin")
                  {
                 ?>
                	<tr>
                ...
                <?php	
                 ...
                 ?>
                  <tr>
                    <thead> 
                      ...
                      Date : </strong>
                      <input style="padding:1%" type="date" name="date1" value="<?php $displayDate = date('Y-m-d', strtotime($_SESSION['date']));  echo $displayDate; ?>"  /></th>
                    </thead>  
                  </tr>
                  <tr>
                    <thead> 
                  ...
                  while($row=mysqli_fetch_array($q1))
                  {
                    $exp="exp_".$row['ing_id'];
                    $bat="bat_".$row['ing_id'];
                    $in="select *from  traceability_cooking ORDER BY trac_ck_id DESC LIMIT 1";
                    //echo $in;
                 ...
                 ?>
                  <tr>
                    ...
                    <td><input type="date" value="<?php echo $in_row[$exp]; ?>" name="exp_<?php echo $row['ing_id']; ?>" min="<?php echo $date; ?>"  class="form-control" required style="border:1;border-color:<?php echo $color; ?>"/></td>
                    <td><input type="text" value="<?php echo $in_row[$bat]; ?>" name="bat_<?php echo $row['ing_id']; ?>"  class="form-control" required style="border:1;border-color:<?php echo $color; ?>"/></td>
